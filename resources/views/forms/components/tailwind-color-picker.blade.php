                ...
                <div class="flex items-center">
                    ...
                    <div class=" ml-3 mt-8 w-96">
                        ...
                        <div x-show="isOpen" @click.away="isOpen = false"
                            ...
                            class="origin-top-right absolute mt-4 bg-stone-50 rounded-md shadow-lg z-50 border-4 border-gray-500">
                            <div class="rounded-md shadow-xs w-[28rem]">
                                <div class="text-xs text-gray-900 text-center w-full pt-2">Preview zone</span></div>
                                <div class="rounded-md border-2 border-dashed border-black h-24 m-4">
                                    <div class="w-full h-full rounded-md"
                                         :style="{ 'background-color': colorSelectedHex }"
                                    >
                                    </div>
                                </div>
                                <div class="grid grid-cols-10 pt-2 border-t-4 border-gray-500 bg-gradient-to-r from-gray-100 to-white ">
                                    <template x-for="color in colors" :key="color.twLabel">
                                        <div class="px-2">
                                            <template x-if="colorSelected === color.twLabel">
                                                <div class="w-8 h-8 inline-flex rounded-full cursor-pointer border-2 border-black border-dashed"
                                            ...
                                            <template x-if="colorSelected != color.twLabel">
                                                <div 
                                                    ...
                                                    class="w-8 h-8 inline-flex rounded-full cursor-pointer border-2 border-black border-dashed focus:outline-none focus:shadow-outline"
                                                    :style="{ 'background-color': color.hex }"
                                                    :title="color.twLabel">
                                                </div>
                                            </template>
